...
namespace AltivaWebApp.Repositories
{
    public class CompraRepository : BaseRepository<TbPrCompra>, ICompraRepository
    {
        ...
        public TbPrCompra GetCompraById(int id)
        {
            try
            {
                return context.TbPrCompra.Include(c => c.TbPrCompraDetalle).ThenInclude(cd => cd.IdInventarioNavigation).ThenInclude(i => i.TbPrInventarioBodega).FirstOrDefault(c => c.Id == id);
            }
            catch (Exception)
            {
        ...
        public TbPrCompraDetalle GetCompraDetalleById(long id)
        {
            try
            {
                return context.TbPrCompraDetalle.Include(c => c.IdCompraNavigation).Include(i => i.IdInventarioNavigation).ThenInclude(b => b.TbPrInventarioBodega).FirstOrDefault(c => c.Id == id);
            }
            catch (Exception)
            {
